...
exports.default = {
    ...
    extraConfig2: '.labelization|`object`|不适用|可选,标签化多选结果的配置项,参考示例|',
    extraConfig3: '.labelization.enable|`boolean`|false|可选下的必填参数,是否启用标签化,使用必须置为true,参考示例|',
    extraConfig4: '.labelization.overflow|`string`|\'\'|可选,多个标签超出容器时候的预处理行为,值为`\'normal\'\|\'scroll-y\'\|\'multiple-line\'\|\'string\'`对应默认隐藏,纵向滚动、自动变多行和自定义类|',
    extraConfig5: '.labelization.containnerMaxHeight|`string`|\'1.8em\'|可选,限制容器最高高度。多行模式下默认不限制高度,单行模式下默认为1.8em|',
    extraConfig6: '.labelization.labelMaxWidth|`string`|\'100%\'|可选下,限制标签宽度,默认值为行宽的100%|',
    extraConfig7: '.selectedItemWithTemplate|`object`|不适用|可选,在单选情况下,显示选项使用了template的情况下,顶部选中的内容是否也以template展示,参考示例|',
    extraConfig8: '.selectedItemWithTemplate.enable|`boolean`|无|可选下的必填参数,是否启用选中项使用模板,使用必须置为true,参考示例|',
